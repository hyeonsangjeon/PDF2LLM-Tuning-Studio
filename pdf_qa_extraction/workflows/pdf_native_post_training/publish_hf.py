...
def build_model_card(summary: Dict[str, Any], *, repo_id: str, base_model: str,
                     arm: str, reference_scores: bool = False) -> str:
    """summary.json 의 실측 점수로 정직한 모델 카드(Markdown + YAML front-matter)를 만든다.

    ``reference_scores=True`` 이면 점수표를 **8B 레퍼런스 벤치마크 값**으로 명확히 라벨링해,
    업로드된 모델(예: 소형 CPU 데모)을 8B 실측 점수의 주인공처럼 오인하지 않게 한다.
    """
    seeds = summary.get("seeds") or []
    bench_base = summary.get("base_model") or "Qwen/Qwen3-8B"
...
**PDF2LLM-Tuning-Studio** 파이프라인으로 `{base_model}` 를 합성 한국어 금융 QA 코퍼스로
SFT 파인튜닝한 **자립형** 모델입니다. `AutoModelForCausalLM.from_pretrained("{repo_id}")` 로 바로 로드됩니다.

> ⚠ **이 모델은 소형 데모 변형입니다.** 아래 점수표는 저장소의 **8B 레퍼런스 벤치마크**
> (`{bench_base}`) 실측값이며 **이 업로드 모델 자체의 점수가 아닙니다** — 방법론과
> 재현 경로(로드→실추론)를 보여주기 위한 참조값입니다. 이 데모 모델의 진짜 점수를 보려면
> 저장소의 `make bench` 로 직접 평가하세요.
"""
        score_header = (f"## 📊 참조: 8B 벤치마크 성능 (`{bench_base}`, seeds={seeds}) "
                        "— 이 모델의 점수가 아님")
    else:
        intro = f"""
# {repo_id}
...
"""
        score_header = f"## 측정된 성능 (실제 A100, seeds={seeds})"
    body = intro + f"""
...
def publish(model_dir: str, repo_id: str, *, base_model: str, arm: str,
            ...
            dry_run: bool = False, write_card: bool = True,
            reference_scores: Optional[bool] = None) -> int:
    ...
    bench_base = summary.get("base_model") or "Qwen/Qwen3-8B"
    base_model = base_model or bench_base
    # 업로드 모델의 베이스가 8B 벤치마크와 다르면(예: 소형 CPU 데모) 점수를 '참조값'으로 라벨링.
    if reference_scores is None:
        reference_scores = _norm_model(base_model) != _norm_model(bench_base)
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    ap.add_argument("--reference-scores", dest="reference_scores", action="store_true",
                    default=None,
                    help="점수표를 '8B 벤치마크 참조값'으로 라벨(소형 데모 업로드용). "
                         "기본: 베이스 모델이 8B와 다르면 자동 적용")
