...
make cook-demo && make ask-hf HF=runs/cook_demo  # GPU·토큰 없이 진짜 소형 모델을 굽고 그 가중치로 추론
```

- **따라 하기(클론→학습→질문 3단계)** 와 `make ask`(실제 A100 6-arm 답 오프라인 재생)로
  "입력 → 학습 → 실험 문장 넣으면 짠!"을 GPU 없이 바로 체험 — 가이드의 🎬 섹션 참고.
- **재현 3-tier**: `ask`(0-설정 재생) · `ask --hf`(HF/로컬 **파인튜닝 가중치 실로드→실시간 추론**, GPU 없으면 `make cook-demo` 로 진짜 소형 가중치 생성) · `make bench`(내 GPU 처음부터) —
...
- 실행: `make demo-replay`(오프라인) · `make demo-live-ollama`(로컬 Ollama) · `make demo-train-smoke`(CPU SFT 스모크) · `make ask`(질문→답) · `make ask-hf`(HF 가중치 실로드) · `make cook-demo`(CPU 실파인튜닝) · `make bench`(처음부터 재현) · `make publish-hf`(HF 업로드)
